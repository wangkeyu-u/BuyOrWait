...
def build_benchmark_chart(benchmarks: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#f2f0ea"),
        xaxis_title="Runtime (minutes, placeholder)",
        yaxis_title="",
...
def page_alerts() -> None:
    ...
    with cols[3]:
        alert_card("Latest alert date", latest, "mock data freshness", "medium")
    ...
    st.markdown(
        ...
        <div class="bow-panel">
          ...
          <div class="bow-card-note">
            ...
            {scores.loc[scores.game_name == selected.game_name, "buy_confidence_score"].iloc[0]:.0f}/100.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def page_gpu() -> None:
    ...
    st.markdown('<span class="todo-badge">TODO: replace with real benchmark results.</span>', unsafe_allow_html=True)
    st.write("")
    cols = st.columns(4)
    cols[0].metric("Dataset scale", "100M+ reviews")
    cols[1].metric("CPU pandas time", f"{end_to_end.pandas_cpu_time:.0f} min")
    cols[2].metric("GPU cudf.pandas time", f"{end_to_end.cudf_gpu_time:.0f} min")
    cols[3].metric("End-to-end speedup", f"{end_to_end.speedup:.1f}x")
    st.caption("Hardware note: placeholder CPU baseline vs NVIDIA L4 GPU. Replace after real benchmark run.")
